...
template <class dataType> int CustomVector<dataType>::_id = 0;
...
template <class dataType>
CustomVector<dataType>::CustomVector(const size_t sz, std::string str)
    : _size(sz), _capacity(2 * sz), _data(nullptr), _filePtrName(str) {
...
template <class dataType>
CustomVector<dataType>::CustomVector(const size_t sz, const dataType &val)
    : _size(sz), _capacity(2 * sz) {
  // _size = sz;
  // _capacity = sz;
  _spinlock.lock();
...
CustomVector<dataType>::CustomVector(
    const std::initializer_list<dataType> &i_list)
    : _size(i_list.size()), _capacity(i_list.size() * 2) {
...
// copy constructor
template <class dataType>
CustomVector<dataType>::CustomVector(const CustomVector<dataType> &vec)
...
// destructor
template <class dataType> CustomVector<dataType>::~CustomVector() {
  if (_data == nullptr)
    return;
...
template <class dataType>
CustomVector<dataType> &
CustomVector<dataType>::operator=(const CustomVector<dataType> &vec) {
  _spinlock.lock();
...
// return size
template <class dataType> inline size_t CustomVector<dataType>::size() const {
  return _size;
}

// check whether empty or not
template <class dataType> inline bool CustomVector<dataType>::empty() const {
  return _size == 0;
}
...
// get first element
template <class dataType> inline dataType &CustomVector<dataType>::front() {
  if (this->empty())
...
// get last element
template <class dataType> inline dataType &CustomVector<dataType>::back() {
  if (this->empty())
    throw std::out_of_range("Vector Empty");
...
// iterators
template <class dataType> inline dataType *CustomVector<dataType>::frontPtr() {
  if (!this->empty())
    throw std::out_of_range("Vector Empty");
...
template <class dataType> inline dataType *CustomVector<dataType>::backPtr() {
  if (this->empty())
    throw std::out_of_range("Vector Empty");
...
template <class dataType> void CustomVector<dataType>::clear() {
  _capacity = 0;
  _size = 0;
  _data = nullptr;
}
...
template <class dataType> void CustomVector<dataType>::popBack() {
  if (this->empty())
...
void CustomVector<dataType>::insert(const size_t idx, const dataType &val) {
  ...
  if (idx >= 0 && idx < _size) {
    ...
    if (_size >= _capacity) {
      ...
      } else { // vec empty, inserting at start
        newData[0] = val;
      }
...
template <class T> void CustomVector<T>::erase(const size_t idx) {
  if (idx >= 0 && idx < _size) {
    _spinlock.lock();
...
template <class T> void CustomVector<T>::getFileName() {
  ++_id;
...
template <class T> void CustomVector<T>::saveFileName() {
...
template <class T> void CustomVector<T>::writeToFile() {
  if (_readState == true)
    return;
  else {
...
template <class T> void CustomVector<T>::changeReadState() {
  _spinlock.lock();
