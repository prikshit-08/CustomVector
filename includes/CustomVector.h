...
public:
  // CustomVector();										/* to initialize vector of
  // size 0*/
  CustomVector(const size_t sz = 0,
               std::string str = ""); /* to initialize a vector of size sz with
                                         default argument  = 0 */
  CustomVector(const size_t sz,
               const dataType &val); /* to initialize vector of size sz and
                                        having all elements val*/
  CustomVector(const std::initializer_list<dataType> &
                   i_list); /*to initialize vector from a list e.g v = {1,2,3}*/
  CustomVector(
      const CustomVector<dataType> &); /* copy vector into another vector*/

  ~CustomVector(); /* destructor*/

  CustomVector<dataType> &
  operator=(const CustomVector<dataType> &); /* assignmemt v1 = v2*/
  ...
  void resize(const size_t n,
              const dataType &val);   /* resize vectore and make element = val*/
